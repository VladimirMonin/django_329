...
## Lesson 63: Создание формы для добавления карточек (не связано с моделью)
- Рассмотрение класса ModelForm
- Рендер формы в шаблоне
- Обработка POST запроса
- CSRF токен
- Валидация формы

## Lesson 64: Работа с формами Django
- Формы связанные с моделями (Form, ModelForm)
- Валидация формы
- Метод ModelForm _сlean_ - валидация формы
- Собственный класс валидатор
- Создание формы для добавления карточек
- Создание предпросмотра карточек при загрузке
- Создание поисковой формы для карточек

## Lesson 65
- Ищем служебный шаблон Джанго для добавления карточек
- `.venv/Lib/site-packages/django/contrib/admin/templates/admin/change_form.html`
- Сделали кастомный шаблон для добавления карточек в админке `templates/admin/cards/change_form_custom.html`
- Вклинились в блок `content` и добавили элемент предпросмотра
- Подключили это в адмике через `change_form_template` в `ModelAdmin`
- Подключил дополнительный фильтр "наличие кода"

**commit: `lesson_65: подключение кастомного шаблона в админке`**

- Создали представление для обработки асинхронного запроса для препросмотра карточек `/cards/preview_card_ajax/`
- Используем шаблон для детального тображения карточки
- Добавили это и в админку и в представление добавления карточек


**commit: `lesson_65: добавление предпросмотра карточек в админке и в представлении`**

- Как сделать дамп базы в JSON формате?
- `python -Xutf8 manage.py dumpdata --indent=2 > dump.json` - сделает дамп всех данных в JSON формате
...
### Работа с классми-представлениями
- `class AddCardView(View)` - создание класса представления
- Подключили его в `urls.py` через `as_view()`

**commit: `lesson_65: добавление класса представления для добавления карточек`**

- `class IndexView(TemplateView)` - сделали класс представления для главной страницы и about
- Подключили его в `urls.py` через `as_view()`
...
- Переписали login logout на классы
- Починили частичное кеширование страницы каталога карточек (см шаблоны, тег `{% cache %}`)
...
#TODO - добавить автора и теги к предпросмотру карточек при добавлении/редактировании карточек
...
#TODO - добавить новые шаблоны в конспект `##Профиль пользователя`

**commit: `lesson_70: оптимизация шаблонов профиля`**

- Работа с восстановлением пароля
...
#TODO - конспект в этом месте крайне плох. Код рабочий. Надо взять код и переписать конспект


**commit: `lesson_71: восстановление пароля пользователя`**


- Создали модель пользователя с наследованием от `AbstractUser` в `models.py` в приложении `users`
- Определили там поля биография, аватарка, дата рождения
- Подключили новую модель пользователя в `settings.py` в `AUTH_USER_MODEL`
- Поправили фикстуры, потому что пришлось)
- Создали миграцию и применили ее

**commit: `lesson_71: создание модели пользователя`**